...
var sigmoidDerivative = (x) => 4*Math.E**(-x)/(1+Math.E**(-x))**2
var sigmoidSecondDerivative = (x) => -(4*(Math.E**x)*(Math.E**x-1))/((1+Math.E**(x))**3)

renderBoard = (ctx, canvas) => {
	ctx.beginPath();
	ctx.moveTo(0, mapToY(-2))
...
particle = (x, y, vx, vy, steps, draw, ctx) => {
	...
	if (draw) {
		ctx.stroke();
		ctx.fillRect(mapToX(x)-25/2, mapToY(y)-25/2, 25, 25)
	}
	return [x,y];
}
...
function mapPosition(x, y, steps, d, ctx, canvas) {
	for (var vx = 0; vx < canvas.width; vx+=d) {
		for (var vy = 0; vy < canvas.width; vy+=d) {
			var p = particle(x, y, (vx-960)/100000, (vy-540)/100000, steps);
...
canvas1.onmousemove = (e) => {
	...
	particle(960, 540, (e.clientX-960)/100000, (e.clientY-540)/100000, 3000, true, ctx2);
}
