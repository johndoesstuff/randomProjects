...
var sigmoidDerivative = (x) => Math.sin(x)+1
var sigmoidSecondDerivative = (x) => Math.cos(x)
...
function frame() {
	...
	sigmoidDerivative = (x) => Math.sin(x+f*2*Math.PI/120)+1
	sigmoidSecondDerivative = (x) => Math.cos(x+f*2*Math.PI/120)
	f++;
	var a = document.createElement("a");
	a.href = canvas1.toDataURL();
	a.download = "sinframe" + String(f).padStart(4,0) + ".png";
	a.click();
}
