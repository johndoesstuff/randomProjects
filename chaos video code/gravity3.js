...
canvas2.width = 1920;
canvas2.height = 1080;
//setInterval(() => {if (f<=300) {frame()}})
var bodies = JSON.parse(JSON.stringify([
	{
		x: 960+200*Math.cos(0*Math.PI),
		y: 540+200*Math.sin(0*Math.PI),
		m: 10,
		color: "black",
	}, {
		x: 960+200*Math.cos(1*Math.PI),
		y: 540+200*Math.sin(1*Math.PI),
		m: 10,
		color: "white",
	},
]));
...
function frame() {
	mapPosition(960, 540, 5, 30 + f, 5, ctx1, canvas1)
	f++;
	var a = document.createElement("a");
	a.href = canvas1.toDataURL();
	a.download = "2pointnodampstep" + String(f).padStart(4,0) + ".png";
	a.click();
}
...
function mapVelocity(vx, vy, m, steps, d, ctx, canvas) {
	for (var x = 0; x < canvas.width; x+=d) {
		for (var y = 0; y < canvas.width; y+=d) {
			...
			//ctx.fillStyle = "rgb(" + [255-255*Math.hypot(p[0]-960, p[1]-540)/960, 255-255*Math.hypot(p[0]-960, p[1]-540)/960, 255-255*Math.hypot(p[0]-960, p[1]-540)/960].join() + ")"
